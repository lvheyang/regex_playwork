 ...
 * an instance of this class.  The resulting pattern can then be used to create a {@link Matcher}
 * object that can match arbitrary {@linkplain CharSequence character sequences} against the regular
 * expression.  All of the state involved in performing a match resides in the matcher, so many
 ...
 * convenience for when a regular expression is used just once.  This method compiles an expression
 * and matches an input sequence against it in a single invocation.  The statement
 *
 * <blockquote><pre>
 ...
 * concurrent threads.  Instances of the {@link Matcher} class are not safe for such use.
 *
 *
 ...
 * (0&nbsp;{@code <=}&nbsp;<i>m</i>&nbsp;{@code <=}&nbsp;3, 0&nbsp;{@code
 * <=}&nbsp;<i>n</i>&nbsp;{@code <=}&nbsp;7)</td></tr>
 * <tr><th style="vertical-align:top; font-weight: normal" id="hex_hh">{@code \x}<i>hh</i></th>
 * <td headers="matches characters hex_hh">The character with hexadecimal value {@code
 * 0x}<i>hh</i></td></tr>
 * <tr><th style="vertical-align:top; font-weight: normal" id="hex_hhhh"><code>&#92;u</code><i>hhhh</i></th>
 * <td headers="matches characters hex_hhhh">The character with hexadecimal&nbsp;value&nbsp;{@code
 * 0x}<i>hhhh</i></td></tr>
 * <tr><th style="vertical-align:top; font-weight: normal" id="hex_h_h"><code>&#92;x</code><i>{h...h}</i></th>
 * <td headers="matches characters hex_h_h">The character with hexadecimal value {@code
 * 0x}<i>h...h</i>
 * ({@link Character#MIN_CODE_POINT Character.MIN_CODE_POINT} &nbsp;&lt;=&nbsp;{@code
 * 0x}<i>h...h</i>&nbsp;&lt;=&nbsp; {@link Character#MAX_CODE_POINT Character.MAX_CODE_POINT})</td></tr>
 * <tr><th style="vertical-align:top; font-weight: normal" id="unicode_name"><code>&#92;N{</code><i>name</i><code>}</code></th>
 * <td headers="matches characters unicode_name">The character with Unicode character name
 * <i>'name'</i></td></tr>
 ...
 * <td headers="matches characters newline">The newline (line feed) character
 * (<code>'&#92;u000A'</code>)</td></tr>
 * <tr><th style="vertical-align:top; font-weight:normal" id="return">{@code \r}</th>
 * <td headers="matches characters return">The carriage-return character
 * (<code>'&#92;u000D'</code>)</td></tr>
 * <tr><th style="vertical-align:top; font-weight:normal" id="form_feed">{@code \f}</th>
 * <td headers="matches characters form_feed">The form-feed character
 * (<code>'&#92;u000C'</code>)</td></tr>
 ...
 * <td headers="matches characters ctrl_x">The control character corresponding to
 * <i>x</i></td></tr>
 *
 ...
 * <td headers="matches classes negation">Any character except {@code a}, {@code b}, or {@code c}
 * (negation)</td></tr>
 ...
 * <tr><th style="vertical-align:top; font-weight:normal" id="intersection">{@code
 * [a-z&&[def]]}</th>
 * <td headers="matches classes intersection">{@code d}, {@code e}, or {@code f}
 * (intersection)</tr>
 * <tr><th style="vertical-align:top; font-weight:normal" id="subtraction1">{@code
 * [a-z&&[^bc]]}</th>
 * <td headers="matches classes subtraction1">{@code a} through {@code z},
 * except for {@code b} and {@code c}: {@code [ad-z]} (subtraction)</td></tr>
 * <tr><th style="vertical-align:top; font-weight:normal" id="subtraction2">{@code
 * [a-z&&[^m-p]]}</th>
 ...
 * <td headers="matches predef any">Any character (may or may not match <a href="#lt">line
 * terminators</a>)</td></tr>
 ...
 * <td headers="matches predef non_horiz_white">A non-horizontal whitespace character: {@code
 * [^\h]}</td></tr>
 ...
 * <td headers="matches predef vert_white">A vertical whitespace character:
 * <code>[\n\x0B\f\r\x85&#92;u2028&#92;u2029]</code>
 * </td></tr>
 * <tr><th style="vertical-align:top; font-weight:normal" id="non_vert_white">{@code \V}</th>
 * <td headers="matches predef non_vert_white">A non-vertical whitespace character: {@code
 * [^\v]}</td></tr>
 ...
 *
 * <tr><th colspan="2" style="padding-top:20px" id="posix"><b>POSIX character classes (US-ASCII
 * only)</b></th></tr>
 *
 ...
 * <td headers="matches posix Alnum">An alphanumeric character:{@code
 ...
 *
 * <tr><th colspan="2" style="padding-top:20px" id="java">java.lang.Character classes (simple <a
 * href="#jcc">java character type</a>)</th></tr>
 *
 * <tr><th style="vertical-align:top; font-weight:normal" id="javaLowerCase">{@code
 * \p{javaLowerCase}}</th>
 * <td headers="matches java javaLowerCase">Equivalent to java.lang.Character.isLowerCase()</td></tr>
 * <tr><th style="vertical-align:top; font-weight:normal" id="javaUpperCase">{@code
 * \p{javaUpperCase}}</th>
 * <td headers="matches java javaUpperCase">Equivalent to java.lang.Character.isUpperCase()</td></tr>
 * <tr><th style="vertical-align:top; font-weight:normal" id="javaWhitespace">{@code
 * \p{javaWhitespace}}</th>
 * <td headers="matches java javaWhitespace">Equivalent to java.lang.Character.isWhitespace()</td></tr>
 * <tr><th style="vertical-align:top; font-weight:normal" id="javaMirrored">{@code
 * \p{javaMirrored}}</th>
 * <td headers="matches java javaMirrored">Equivalent to java.lang.Character.isMirrored()</td></tr>
 *
 * <tr><th colspan="2" style="padding-top:20px"  id="unicode">Classes for Unicode scripts, blocks,
 * categories and binary properties</th></tr>
 *
 * <tr><th style="vertical-align:top; font-weight:normal" id="IsLatin">{@code \p{IsLatin}}</th>
 * <td headers="matches unicode IsLatin">A Latin&nbsp;script character (<a
 * href="#usc">script</a>)</td></tr>
 * <tr><th style="vertical-align:top; font-weight:normal" id="InGreek">{@code \p{InGreek}}</th>
 * <td headers="matches unicode InGreek">A character in the Greek&nbsp;block (<a
 ...
 * <tr><th style="vertical-align:top; font-weight:normal" id="IsAlphabetic">{@code
 * \p{IsAlphabetic}}</th>
 * <td headers="matches unicode IsAlphabetic">An alphabetic character (<a href="#ubpc">binary
 * property</a>)</td></tr>
 ...
 * <td headers="matches unicode not_InGreek">Any character except one in the Greek block
 * (negation)</td></tr>
 * <tr><th style="vertical-align:top; font-weight:normal" id="not_uppercase">{@code
 * [\p{L}&&[^\p{Lu}]]}</th>
 * <td headers="matches unicode not_uppercase">Any letter except an uppercase letter
 * (subtraction)</td></tr>
 *
 ...
 * \b{g}}</th>
 * <td headers="matches bounds grapheme_cluster_boundary">A Unicode extended grapheme cluster
 * boundary</td></tr>
 ...
 * <tr><th style="vertical-align:top; font-weight:normal" id="end_input_except_term">{@code
 * \Z}</th>
 ...
 *
 * <tr><th style="vertical-align:top; font-weight:normal" id="any_unicode_linebreak">{@code
 * \R}</th>
 * <td headers="matches linebreak any_unicode_linebreak">Any Unicode linebreak sequence, is
 * equivalent to
 * <code>&#92;u000D&#92;u000A|[&#92;u000A&#92;u000B&#92;u000C&#92;u000D&#92;u0085&#92;u2028&#92;u2029]
 * </code></td></tr>
 *
 * <tr><th colspan="2" style="padding-top:20px" id="grapheme">Unicode Extended Grapheme
 * matcher</th></tr>
 *
 ...
 *
 * <tr><th style="vertical-align:top; font-weight:normal" id="greedy_once_or_not"><i>X</i>{@code
 * ?}</th>
 * <td headers="matches greedy greedy_once_or_not"><i>X</i>, once or not at all</td></tr>
 * <tr><th style="vertical-align:top; font-weight:normal" id="greedy_zero_or_more"><i>X</i>{@code
 * *}</th>
 * <td headers="matches greedy greedy_zero_or_more"><i>X</i>, zero or more times</td></tr>
 * <tr><th style="vertical-align:top; font-weight:normal" id="greedy_one_or_more"><i>X</i>{@code
 * +}</th>
 ...
 * <tr><th style="vertical-align:top; font-weight:normal" id="greedy_at_least"><i>X</i><code>{</code><i>n</i>{@code
 * ,}}</th>
 * <td headers="matches greedy greedy_at_least"><i>X</i>, at least <i>n</i> times</td></tr>
 * <tr><th style="vertical-align:top; font-weight:normal" id="greedy_at_least_up_to"><i>X</i><code>{</code><i>n</i>{@code
 * ,}<i>m</i><code>}</code></th>
 * <td headers="matches greedy greedy_at_least_up_to"><i>X</i>, at least <i>n</i> but not more than
 * <i>m</i> times</td></tr>
 *
 * <tr><th colspan="2" style="padding-top:20px" id="reluc">Reluctant quantifiers</th></tr>
 *
 * <tr><th style="vertical-align:top; font-weight:normal" id="reluc_once_or_not"><i>X</i>{@code
 * ??}</th>
 * <td headers="matches reluc reluc_once_or_not"><i>X</i>, once or not at all</td></tr>
 * <tr><th style="vertical-align:top; font-weight:normal" id="reluc_zero_or_more"><i>X</i>{@code
 * *?}</th>
 * <td headers="matches reluc reluc_zero_or_more"><i>X</i>, zero or more times</td></tr>
 * <tr><th style="vertical-align:top; font-weight:normal" id="reluc_one_or_more"><i>X</i>{@code
 * +?}</th>
 ...
 * <tr><th style="vertical-align:top; font-weight:normal" id="reluc_at_least_up_to"><i>X</i><code>{</code><i>n</i>{@code
 * ,}<i>m</i><code>}?</code></th>
 * <td headers="matches reluc reluc_at_least_up_to"><i>X</i>, at least <i>n</i> but not more than
 * <i>m</i> times</td></tr>
 *
 * <tr><th colspan="2" style="padding-top:20px" id="poss">Possessive quantifiers</th></tr>
 *
 * <tr><th style="vertical-align:top; font-weight:normal" id="poss_once_or_not"><i>X</i>{@code
 * ?+}</th>
 * <td headers="matches poss poss_once_or_not"><i>X</i>, once or not at all</td></tr>
 * <tr><th style="vertical-align:top; font-weight:normal" id="poss_zero_or_more"><i>X</i>{@code
 * *+}</th>
 * <td headers="matches poss poss_zero_or_more"><i>X</i>, zero or more times</td></tr>
 * <tr><th style="vertical-align:top; font-weight:normal" id="poss_one_or_more"><i>X</i>{@code
 * ++}</th>
 ...
 * <tr><th style="vertical-align:top; font-weight:normal" id="poss_at_least_up_to"><i>X</i><code>{</code><i>n</i>{@code
 * ,}<i>m</i><code>}+</code></th>
 * <td headers="matches poss poss_at_least_up_to"><i>X</i>, at least <i>n</i> but not more than
 * <i>m</i> times</td></tr>
 *
 ...
 * <tr><th style="vertical-align:top; font-weight:normal" id="alternate"><i>X</i>{@code
 * |}<i>Y</i></th>
 * <td headers="matches logical alternate">Either <i>X</i> or <i>Y</i></td></tr>
 * <tr><th style="vertical-align:top; font-weight:normal" id="group">{@code (}<i>X</i>{@code
 * )}</th>
 ...
 * <tr><th style="vertical-align:top; font-weight:normal" id="back_named">{@code
 * \}<i>k</i>&lt;<i>name</i>&gt;</th>
 ...
 * <td headers="matches quote quote_begin">Nothing, but quotes all characters until {@code
 * \E}</td></tr>
 ...
 * <!-- Metachars: !$()*+.<>?[\]^{|} -->
 *
 * <tr><th colspan="2" style="padding-top:20px" id="special">Special constructs (named-capturing and
 * non-capturing)</th></tr>
 *
 * <tr><th style="vertical-align:top; font-weight:normal" id="named_group"><code>(?&lt;<a
 * href="#groupname">name</a>&gt;</code><i>X</i>{@code )}</th>
 * <td headers="matches special named_group"><i>X</i>, as a named-capturing group</td></tr>
 * <tr><th style="vertical-align:top; font-weight:normal" id="non_capture_group">{@code
 * (?:}<i>X</i>{@code )}</th>
 ...
 * <td headers="matches special flags">Nothing, but turns match flags <a
 * href="#CASE_INSENSITIVE">i</a>
 * <a href="#UNIX_LINES">d</a> <a href="#MULTILINE">m</a> <a href="#DOTALL">s</a>
 * <a href="#UNICODE_CASE">u</a> <a href="#COMMENTS">x</a> <a href="#UNICODE_CHARACTER_CLASS">U</a>
 * on - off</td></tr>
 * <tr><th style="vertical-align:top; font-weight:normal" id="non_capture_group_flags"><code>(?idmsux-idmsux:</code><i>X</i>{@code
 * )}&nbsp;&nbsp;</th>
 * <td headers="matches special non_capture_group_flags"><i>X</i>, as a <a href="#cg">non-capturing
 * group</a> with the
 * given flags <a href="#CASE_INSENSITIVE">i</a> <a href="#UNIX_LINES">d</a>
 * <a href="#MULTILINE">m</a> <a href="#DOTALL">s</a> <a href="#UNICODE_CASE">u</a >
 * <a href="#COMMENTS">x</a> on - off</td></tr>
 * <tr><th style="vertical-align:top; font-weight:normal" id="pos_lookahead">{@code
 * (?=}<i>X</i>{@code )}</th>
 * <td headers="matches special pos_lookahead"><i>X</i>, via zero-width positive
 * lookahead</td></tr>
 * <tr><th style="vertical-align:top; font-weight:normal" id="neg_lookahead">{@code
 * (?!}<i>X</i>{@code )}</th>
 * <td headers="matches special neg_lookahead"><i>X</i>, via zero-width negative
 * lookahead</td></tr>
 * <tr><th style="vertical-align:top; font-weight:normal" id="pos_lookbehind">{@code
 * (?<=}<i>X</i>{@code )}</th>
 * <td headers="matches special pos_lookbehind"><i>X</i>, via zero-width positive
 * lookbehind</td></tr>
 * <tr><th style="vertical-align:top; font-weight:normal" id="neg_lookbehind">{@code
 * (?<!}<i>X</i>{@code )}</th>
 * <td headers="matches special neg_lookbehind"><i>X</i>, via zero-width negative
 * lookbehind</td></tr>
 * <tr><th style="vertical-align:top; font-weight:normal" id="indep_non_capture_group">{@code
 * (?>}<i>X</i>{@code )}</th>
 * <td headers="matches special indep_non_capture_group"><i>X</i>, as an independent, non-capturing
 * group</td></tr>
 *
 * </tbody>
 * </table>
 ...
 * constructs, as defined in the table above, as well as to quote characters that otherwise would be
 * interpreted as unescaped constructs.  Thus the expression {@code \\} matches a single backslash
 * and <code>\{</code> matches a left brace.
 *
 * <p> It is an error to use a backslash prior to any alphabetic character that
 * does not denote an escaped construct; these are reserved for future extensions to the
 * regular-expression language.  A backslash may be used prior to a non-alphabetic character
 * regardless of whether that character is part of an unescaped construct.
 *
 * <p> Backslashes within string literals in Java source code are interpreted
 * as required by
 * <cite>The Java&trade; Language Specification</cite>
 * as either Unicode escapes (section 3.3) or other character escapes (section 3.10.6) It is
 * therefore necessary to double backslashes in string literals that represent regular expressions
 * to protect them from interpretation by the Java bytecode compiler.  The string literal
 * <code>"&#92;b"</code>, for example, matches a single backspace character when
 * interpreted as a regular expression, while {@code "\\b"} matches a word boundary.  The string
 * literal {@code "\(hello\)"} is illegal and leads to a compile-time error; in order to match the
 * string {@code (hello)} the string literal {@code "\\(hello\\)"} must be used.
 *
 * <h3><a id="cc">Character Classes</a></h3>
 *
 * <p> Character classes may appear within other character classes, and
 * may be composed by the union operator (implicit) and the intersection operator ({@code &&}). The
 * union operator denotes a class that contains every character that is in at least one of its
 * operand classes.  The intersection operator denotes a class that contains every character that is
 * in both of its operand classes.
 *
 ...
 * a character class than outside a character class. For instance, the regular expression {@code .}
 * loses its special meaning inside a character class, while the expression {@code -} becomes a
 * range forming metacharacter.
 *
 * <h3><a id="lt">Line terminators</a></h3>
 *
 * <p> A <i>line terminator</i> is a one- or two-character sequence that marks
 * the end of a line of the input character sequence.  The following are recognized as line
 * terminators:
 *
 * <ul>
 *
 ...
 * line terminators and only match at the beginning and the end, respectively, of the entire input
 * sequence. If {@link #MULTILINE} mode is activated then {@code ^} matches at the beginning of
 * input and after any line terminator except at the end of input. When in {@link #MULTILINE} mode
 * {@code $} matches just before a line terminator or the end of the input sequence.
 *
 * <h3><a id="cg">Groups and capturing</a></h3>
 *
 * <h4><a id="gnumber">Group number</a></h4>
 * <p> Capturing groups are numbered by counting their opening parentheses from
 * left to right.  In the expression {@code ((A)(B(C)))}, for example, there are four such groups:
 * </p>
 *
 ...
 * of the input sequence that matches such a group is saved.  The captured subsequence may be used
 * later in the expression, via a back reference, and may also be retrieved from the matcher once
 * the match operation is complete.
 *
 * <h4><a id="groupname">Group name</a></h4>
 * <p>A capturing group can also be assigned a "name", a {@code named-capturing group},
 * and then be back-referenced later by the "name". Group names are composed of the following
 * characters. The first character must be a {@code letter}.
 *
 * <ul>
 ...
 * that the group most recently matched.  If a group is evaluated a second time because of
 * quantification then its previously-captured value, if any, will be retained if the second
 * evaluation fails.  Matching the string {@code "aba"} against the expression {@code (a(b)?)+}, for
 * example, leaves group two set to {@code "b"}.  All captured input is discarded at the beginning
 * of each match.
 *
 ...
 * href="http://www.unicode.org/reports/tr18/"><i>Unicode Technical Standard #18: Unicode Regular
 * Expression</i></a>, plus RL2.1 Canonical Equivalents.
 * <p>
 * <b>Unicode escape sequences</b> such as <code>&#92;u2014</code> in Java source code
 * are processed as described in section 3.3 of
 * <cite>The Java&trade; Language Specification</cite>.
 * Such escape sequences are also implemented directly by the regular-expression parser so that
 * Unicode escapes can be used in expressions that are read from files or from the keyboard.  Thus
 * the strings <code>"&#92;u2014"</code> and {@code "\\u2014"}, while not equal, compile into the
 * same pattern, which matches the character with hexadecimal value {@code 0x2014}.
 * <p>
 * A Unicode character can also be represented by using its <b>Hex notation</b> (hexadecimal code
 * point value) directly as described in construct
 * <code>&#92;x{...}</code>, for example a supplementary character U+2011F can be
 * specified as <code>&#92;x{2011F}</code>, instead of two consecutive Unicode escape sequences of
 * the surrogate pair <code>&#92;uD840</code><code>&#92;uDD1F</code>.
 * <p>
 ...
 * specifies character <code>&#92;u263A</code>. The character names supported by this class are the
 * valid Unicode character names matched by {@link Character#codePointOf(String)
 ...
 * <p>
 * Unicode scripts, blocks, categories and binary properties are written with the {@code \p} and
 * {@code \P} constructs as in Perl.
 * <code>\p{</code><i>prop</i><code>}</code> matches if
 * the input has the property <i>prop</i>, while <code>\P{</code><i>prop</i><code>}</code> does not
 * match if the input has that property.
 * <p>
 * Scripts, blocks, categories and binary properties can be used both inside and outside of a
 * character class.
 *
 * <p>
 ...
 * {@code script=Hiragana} or {@code sc=Hiragana}.
 * <p>
 * The script names supported by {@code Pattern} are the valid script names accepted and defined by
 ...
 * {@code InMongolian}, or by using the keyword {@code block} (or its short form {@code blk}) as in
 * {@code block=Mongolian} or {@code blk=Mongolian}.
 * <p>
 * The block names supported by {@code Pattern} are the valid block names accepted and defined by
 ...
 * Both {@code \p{L}} and {@code \p{IsL}} denote the category of Unicode letters. Same as scripts
 * and blocks, categories can also be specified by using the keyword {@code general_category} (or
 * its short form {@code gc}) as in {@code general_category=Lu} or {@code gc=Lu}.
 * <p>
 * The supported categories are those of
 * <a href="http://www.unicode.org/unicode/standard/standard.html">
 * <i>The Unicode Standard</i></a> in the version specified by the
 * {@link Character Character} class. The category names are those defined in the Standard, both
 ...
 * </ul>
 * <p>
 * The following <b>Predefined Character classes</b> and <b>POSIX character classes</b> are in
 * conformance with the recommendation of <i>Annex C: Compatibility Properties</i> of <a
 * href="http://www.unicode.org/reports/tr18/"><i>Unicode Regular Expression
 ...
 * <p>
 * <a id="jcc">
 * Categories that behave like the java.lang.Character boolean is<i>methodname</i> methods (except
 * for the deprecated ones) are available through the same <code>\p{</code><i>prop</i><code>}</code>
 * syntax where the specified property has the name <code>java<i>methodname</i></code></a>.
 *
 ...
 * {@code (?(}<i>condition</i>{@code )}<i>X</i>{@code )} and {@code (?(}<i>condition</i>{@code
 * )}<i>X</i>{@code |}<i>Y</i>{@code )},
 * </p></li>
 *
 ...
 * as back references; a backslash-escaped number greater than {@code 9} is treated as a back
 * reference if at least that many subexpressions exist, otherwise it is interpreted, if possible,
 * as an octal escape.  In this class octal escapes must always begin with a zero. In this class,
 * {@code \1} through {@code \9} are always interpreted as back references, and a larger number is
 * accepted as a back reference if at least that many subexpressions exist at that point in the
 * regular expression, otherwise the parser will drop digits until the number is smaller or equal to
 * the existing number of groups or it is one digit.
 * </p></li>
 *
 * <li><p> Perl uses the {@code g} flag to request a match that resumes
 * where the last match left off.  This functionality is provided implicitly by the {@link Matcher}
 * class: Repeated invocations of the {@link Matcher#find find} method will resume where the last
 * match left off, unless the matcher is reset.  </p></li>
 *
 * <li><p> In Perl, embedded flags at the top level of an expression affect
 * the whole expression.  In this class, embedded flags always take effect at the point at which
 * they appear, whether they are at the top level or within a group; in the latter case, flags are
 * restored at the end of the group just as in Perl.  </p></li>
 *
 * </ul>
 *
 *
...
public final class Pattern
   ...
   * in the US-ASCII charset are being matched.  Unicode-aware case-insensitive matching can be
   * enabled by specifying the {@link #UNICODE_CASE} flag in conjunction with this flag.
   *
   ...
   * the pattern is treated as a sequence of literal characters. Metacharacters or escape sequences
   ...
   * expression&nbsp;{@code (?s)}.  (The {@code s} is a mnemonic for "single-line" mode, which is
   * what this is called in Perl.)  </p>
   */
   ...
   * enabled by the {@link #CASE_INSENSITIVE} flag, is done in a manner consistent with the Unicode
   * Standard.  By default, case-insensitive matching assumes that only characters in the US-ASCII
   ...
   * to match if, and only if, their full canonical decompositions match. The expression
   * <code>"a&#92;u030A"</code>, for example, will match the string <code>"&#92;u00E5"</code> when
   * this flag is specified.  By default, matching does not take canonical equivalence into
   * account.
   *
  ...
  /**
   * The root of object tree for a match operation.  The pattern is matched at the beginning.  This
   * may include a find that uses BnM or a First node.
   */
  ...
  /**
   * The number of top greedy closure nodes in this Pattern. Used by matchers to allocate storage
   * needed for a IntHashSet to keep the beginning pos {@code i} of all failed match.
   */
  ...
  /**
   * The number of capturing groups in this Pattern. Used by matchers to allocate storage needed to
   * perform a match.
   */
  transient int capturingGroupCount;
  /**
   * The local variable count used by parsing tree. Used by matchers to allocate storage needed to
   * perform a match.
   */
  ...
  private int flags;
  /**
   * Boolean indicating this Pattern is compiled; this is necessary in order to lazily compile
  ...
  /**
   * If the Start node might possibly match supplementary characters. It is set to true during
   * compiling if (1) There is supplementary char in pattern, or (2) There is complement node of a
   * "family" CharProperty
   */
  private transient boolean hasSupplementary;

  /**
   * This private constructor is used to create all Patterns. The pattern string and match flags are
   * all that is needed to completely describe a Pattern. An empty pattern string results in an
   * object tree with only a Start node and a LastNode node.
   */
  ...
  /**
   * Returns a literal pattern {@code String} for the specified {@code String}.
   *
   * <p>This method produces a {@code String} that can be used to
   * create a {@code Pattern} that would match the string {@code s} as if it were a literal
   * pattern.</p> Metacharacters or escape sequences in the input sequence will be given no special
   * meaning.
   *
  ...
  /**
   * Attempts to compose input by combining the first character with the first combining mark
   * following it. Returns a String that is the composition of the leading character with its first
   * combining mark followed by the remaining combining marks. Returns null if the first two
   * characters cannot be further composed.
   */
  private static String composeOneStep(String input) {
    int len = countChars(input, 0, 2);
    String firstTwoCharacters = input.substring(0, len);
    String result = Normalizer.normalize(firstTwoCharacters, Form.NFC);
  ...
  /**
   * Non spacing marks only count as word characters in bounds calculations if they have a base
   * character.
   */
   ...
   * is the regular expression from which this pattern was compiled.</p>
   *
   ...
   * input sequence that is terminated by another subsequence that matches this pattern or is
   * terminated by the end of the input sequence.  The substrings in the array are in the order in
   * which they occur in the input. If this pattern does not match any subsequence of the input then
   * the resulting array has just one element, namely the input sequence in string form.
   *
   * <p> When there is a positive-width match at the beginning of the input
   * sequence then an empty leading substring is included at the beginning of the resulting array. A
   * zero-width match at the beginning however never produces such empty leading substring.
   *
   * <p> The {@code limit} parameter controls the number of times the
   * pattern is applied and therefore affects the length of the resulting array.  If the limit
   * <i>n</i> is greater than zero then the pattern will be applied at most <i>n</i>&nbsp;-&nbsp;1
   * times, the array's length will be no greater than <i>n</i>, and the array's last entry will
   * contain all input beyond the last matched delimiter.  If <i>n</i> is non-positive then the
   * pattern will be applied as many times as possible and the array can have any length.  If
   * <i>n</i> is zero then the pattern will be applied as many times as possible, the array can have
   ...
   * #split(CharSequence, int) split} method with the given input sequence and a limit argument of
   * zero.  Trailing empty strings are therefore not included in the resulting array. </p>
   *
   * <p> The input {@code "boo:and:foo"}, for example, yields the following
  ...
  /**
   * Recompile the Pattern instance from a stream.  The original pattern string is read in and the
   * object tree is recompiled from it.
   */
  ...
  /**
   * Preprocess any \Q...\E sequences in `temp', meta-quoting them. See the description of
   * `quotemeta' in perlfunc(1).
   */
  ...
  /**
   * Copies regular expression to an int array and invokes the parsing of the expression which will
  ...
  /**
   * Internal method used for handling all syntax errors. The pattern is displayed with a pointer to
   * aid in locating the syntax error.
   */
  ...
  /**
   * Determines if there is any supplementary character or unpaired surrogate in the specified
   * range.
   */
  ...
  /**
   * The expression is parsed with branch nodes added for alternations. This may be called
   * recursively to parse sub expressions that may contain alternations.
   */
  ...
  /**
   * Parses a backref greedily, taking as many numbers as it can. The first digit is always treated
   * as a backref, but multi digit numbers are only treated as a backref if at least that many
   * backrefs exist at this point in the regex.
   */
   ...
   * returned and create was true a new object was added to the tree to handle the escape sequence.
   * If the returned value is greater than zero, it is the value that matches the escape sequence.
   */
    ...
    switch (ch) {
      ...
      case 'S':
        if (create) {
          predicate = has(UNICODE_CHARACTER_CLASS) ?
              CharPredicates
                  .WHITE_SPACE() : CharPredicates.ASCII_SPACE();
          predicate = predicate.negate();
          if (!inclass) {
            root = newCharProperty(predicate);
          }
   ...
   *
   * Consumes a ] on the way out if consume is true. Usually consume is true except for the case of
   * [abc&&def] where def is a separate right hand node with "understood" brackets.
   */
  ...
  /**
   * Parses a group and returns the head node of a set of nodes that process the group. Sometimes a
   * double return system is used where the tail is returned in root.
   */
  private Node group0() {
   ...
   * appended to handle the repetition. Prev could be a single or a group, so it could be a chain of
   * nodes.
   */
   ...
   * input sequence that is terminated by another subsequence that matches this pattern or is
   * terminated by the end of the input sequence.  The substrings in the stream are in the order in
   * which they occur in the input. Trailing empty strings will be discarded and not encountered in
   * the stream.
   *
   * <p> If this pattern does not match any subsequence of the input then
   * the resulting stream has just one element, namely the input sequence in string form.
   *
   * <p> When there is a positive-width match at the beginning of the input
   * sequence then an empty leading substring is included at the beginning of the stream. A
   * zero-width match at the beginning however never produces such empty leading substring.
   *
   * <p> If the input sequence is mutable, it must remain constant during the
   * execution of the terminal stream operation.  Otherwise, the result of the terminal stream
  ...
  /**
   * Used to accumulate information about a subtree of the object graph so that optimizations can be
   * applied to the subtree.
   */
  ...
  /**
   * Creates a bit vector for matching Latin-1 values. A normal BitClass never matches values above
   * Latin-1, and a complemented BitClass always matches values above Latin-1.
   */
  ...
  /**
   * Base class for all node classes. Subclasses should override the match() method as appropriate.
   * This class is an accepting node, so its match() always returns true.
   */
  ...
  static class LastNode extends Node {
    ...
    /**
     * This method implements the classic accept node with the addition of a check to see if the
     * match occurred using all of the input.
     */
    boolean match(Matcher matcher, int i, CharSequence seq) {
  ...
  /**
   * Used for REs that can start anywhere within the input string. This basically tries to match
   * repeatedly at each spot in the input string, moving forward after each try. An anchored search
  ...
  /**
   * Node to anchor at the beginning of input. This object implements the match for a \A sequence,
   * and the caret anchor will use this if not in multiline mode.
   */
  ...
  /**
   * Node to anchor at the end of input. This is the absolute end, so this should not match at the
   * last newline before the end as $ will.
   */
  ...
  /**
   * Node to anchor at the beginning of a line. This is essentially the object to match for the
   * multiline ^.
   */
   ...
   *
   * When not in multiline mode, the $ can only match at the very end of the input, unless the input
   * ends in a line terminator in which it matches right before the last line terminator.
   *
  ...
  /**
   * Node to anchor at the end of a line or the end of input based on the multiline mode when in
   * unix lines mode.
   */
  ...
  /**
   * Optimized version of CharProperty that works only for properties never satisfied by
        ...
        if (i + n == j) {    // single, assume nfc cp
          ...
          while (i + n < j) {
            String nfc = Normalizer.normalize(
                seq.toString().substring(i, j), Form.NFC);
  ...
  /**
   * Node class for a unicode_case_insensitive/BMP-only sequence of literal characters. Uses unicode
   * case folding.
   */
  ...
  /**
   * Node class for a case sensitive sequence of literal characters including supplementary
   * characters.
   */
  ...
  /**
   * Node class for a case insensitive sequence of literal characters including supplementary
   * characters.
   */
  ...
  /**
   * Handles the greedy style repetition with the minimum either be 0 or 1 and the maximum be
   * MAX_REPS, for * and + quantifier.
   */
  ...
  /**
   * Handles the curly-brace style repetition with a specified minimum and maximum occurrences. The
   * * quantifier is handled as a special case. This class handles the three types.
   */
  ...
  /**
   * Handles the curly-brace style repetition with a specified minimum and maximum occurrences in
   * deterministic cases. This is an iterative optimization over the Prolog and Loop system which
   * would handle this in a recursive way. The * quantifier is handled as a special case. If capture
   * is true then this class saves group settings and ensures that groups are unset when backing off
   * of a group match.
   */
  ...
  /**
   * A Guard node at the end of each atom node in a Branch. It serves the purpose of chaining the
   * "match" operation to "next" but not the "study", so we can collect the TreeInfo of each atom
   * node without including the TreeInfo of the "next".
   */
  ...
  /**
   * Handles the branching of alternations. Note this is also used for the ? quantifier to branch
  ...
  /**
   * The GroupHead saves the location where the group begins in the locals and restores them when
   * the match is done.
   *
   * The matchRef is used when a reference to this group is accessed later in the expression. The
   * locals will have a negative value in them to indicate that we do not want to unset the group if
   * the reference doesn't match.
   */
  ...
  /**
   * Recursive reference to a group in the regular expression. It calls matchRef because if the
   * reference fails to match we would not unset the group.
   */
  ...
  /**
   * The GroupTail handles the setting of group beginning and ending locations when groups are
   * successfully matched. It must also be able to unset groups that have to be backed off of.
   *
   * The GroupTail node is also used when a previous group is referenced, and in that case no group
   * information needs to be set.
   */
  ...
  /**
   * Handles the repetition count for a greedy Curly. The matchInit is called from the Prolog to
   * save the index of where the group beginning is stored. A zero length group check occurs in the
  ...
  /**
   * Handles the repetition count for a reluctant Curly. The matchInit is called from the Prolog to
   * save the index of where the group beginning is stored. A zero length group check occurs in the
   * normal match but is skipped in the matchInit.
   */
  ...
  /**
   * Refers to a group in the regular expression. Attempts to match whatever the group referred to
   * last matched.
   */
  ...
  /**
   * Searches until the next instance of its atom. This is useful for finding the atom efficiently
   * without passing an instance of it (greedy problem) and without a lot of wasted search time
   * (reluctant problem).
   */
  ...
  /**
   * Handles word boundaries. Includes a field to allow this one class to deal with the different
   * types of word boundaries we can match. The word characters include underscores, letters, and
   * digits. Non spacing marks can are also part of a word if they have a base character, otherwise
  ...
  /**
   * Attempts to match a slice in the input using the Boyer-Moore string matching algorithm. The
   * algorithm is based on the idea that the pattern can be shifted farther ahead in the search text
   * if it is matched right to left.
   * <p>
   * The pattern is compared to the input one character at a time, from the rightmost character in
   * the pattern to the left. If the characters all match the pattern has been found. If a character
   * does not match, the pattern is shifted right a distance that is the maximum of two functions,
   * the bad character shift and the good suffix shift. This shift moves the attempted match
   * position through the input more quickly than a naive one position at a time check.
   * <p>
   * The bad character shift is based on the character from the text that did not match. If the
   * character does not appear in the pattern, the pattern can be shifted completely beyond the bad
   * character. If the character does occur in the pattern, the pattern can be shifted to line the
   * pattern up with the next occurrence of that character.
   * <p>
   * The good suffix shift is based on the idea that some subset on the right side of the pattern
   * has matched. When a bad character is found, the pattern can be shifted right by the pattern
   * length if the subset does not occur again in pattern, or by the amount of distance to the next
  ...
  static class BnM extends Node {
     ...
     * Only the last seven bits are used to see if chars match; This keeps the tables small and
     * covers the heavily used ASCII range, but occasionally results in an aliased match for the bad
     * character shift.
     */
